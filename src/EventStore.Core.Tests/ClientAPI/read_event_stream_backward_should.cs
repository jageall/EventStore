...
{
    public class read_event_stream_backward_should : IClassFixture<MiniNodeFixture>
    {
        private MiniNode _node;

        public read_event_stream_backward_should(MiniNodeFixture data)
        {
            _node = data.Node;
        }
        ...
        public async Task throw_if_count_le_zero()
        {
            ...
            using (var store = BuildConnection(_node))
            {
                ...
                await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() => store.ReadStreamEventsBackwardAsync(stream, 0, 0, resolveLinkTos: false));
            }
        }
        ...
        public async Task throw_when_got_int_max_value_as_maxcount()
        {
            using (var store = BuildConnection(_node))
            {
                ...
                await Assert.ThrowsAsync<ArgumentException>(() => store.ReadStreamEventsBackwardAsync("foo", StreamPosition.Start, int.MaxValue, resolveLinkTos: false));
