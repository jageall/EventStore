...
namespace EventStore.Core.Tests.Services.Storage.Scavenge
{
    public class when_deleting_single_stream_spanning_through_2_chunks_in_2nd_chunk_in_db_with_3_chunks : ReadIndexTestScenario
    {
        ...
        {
            Fixture.WriteSingleEvent("ES", 0, new string('.', 3000)); // chunk 1
            ...
            var event7prepare = Fixture.WriteDeletePrepare("ES");
            var event7commit = Fixture.WriteDeleteCommit(event7prepare);
            var event7 = new EventRecord(EventNumber.DeletedStream, event7prepare);

            var event9 = Fixture.WriteSingleEvent("ES2", 0, new string('.', 5000), retryOnFail: true); //chunk 3

            Fixture.Scavenge(completeLast: false, mergeChunks: false);
            Fixture.AddStashedValueAssignment(this, instance =>
            {
                instance._event7prepare = event7prepare;
                instance._event7commit = event7commit;
                instance._event7 = event7;
                instance._event9 = event9;
            });
        }
