...
namespace EventStore.Core.Tests.Services.Storage.Scavenge
{
    public class when_stream_is_softdeleted_and_temp_and_all_events_and_metaevents_are_in_one_chunk : ScavengeTestScenario
    {
        ...
        public void the_stream_is_absent_physically()
        {
            ...
            Assert.Empty(ReadIndex.ReadAllEventsForward(new TFPos(0, 0), 1000).Records.Where(x => x.Event.EventStreamId == "test"));
            Assert.Empty(ReadIndex.ReadAllEventsBackward(headOfTf, 1000).Records.Where(x => x.Event.EventStreamId == "test"));
        }
