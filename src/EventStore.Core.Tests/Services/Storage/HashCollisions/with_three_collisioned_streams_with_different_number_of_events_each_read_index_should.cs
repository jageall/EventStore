...
namespace EventStore.Core.Tests.Services.Storage.HashCollisions
{
    public class with_three_collisioned_streams_with_different_number_of_events_each_read_index_should : ReadIndexTestScenario
    {
        ...
        {
            var prepares1 = new EventRecord[3];
            for (int i = 0; i < prepares1.Length; i++)
            {
                prepares1[i] = Fixture.WriteSingleEvent("AB", i, "test" + i);
            }

            var prepares2 = new EventRecord[5];
            for (int i = 0; i < prepares2.Length; i++)
            {
                prepares2[i] = Fixture.WriteSingleEvent("CD", i, "test" + i);
            }

            var prepares3 = new EventRecord[7];
            for (int i = 0; i < prepares3.Length; i++)
            {
                prepares3[i] = Fixture.WriteSingleEvent("EF", i, "test" + i);
            }
            Fixture.AddStashedValueAssignment(this, instance =>
            {
                instance._prepares1 = prepares1;
                instance._prepares2 = prepares2;
                instance._prepares3 = prepares3;
                Assert.NotNull(_prepares1);
                Assert.NotNull(_prepares2);
                Assert.NotNull(_prepares3);
            });
        }
        ...
        public with_three_collisioned_streams_with_different_number_of_events_each_read_index_should(FixtureData fixture) : base(fixture)
        {
        }
    }
}
