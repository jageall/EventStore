...
namespace EventStore.Core.Tests.Services.Storage.BuildingIndex
{
    public class when_building_an_index_off_tfile_with_prepares_but_no_commits : ReadIndexTestScenario
    {
        ...
        {
            long p1;
            Fixture.Writer.Write(new PrepareLogRecord(0, Guid.NewGuid(), Guid.NewGuid(), 0, 0, "test1", -1, DateTime.UtcNow,
                                              PrepareFlags.SingleWrite, "type", new byte[0], new byte[0]),
                         out p1);
            long p2;
            Fixture.Writer.Write(new PrepareLogRecord(p1, Guid.NewGuid(), Guid.NewGuid(), p1, 0, "test2", -1, DateTime.UtcNow,
                                              PrepareFlags.SingleWrite, "type", new byte[0], new byte[0]),
                         out p2);
            long p3;
            Fixture.Writer.Write(new PrepareLogRecord(p2, Guid.NewGuid(), Guid.NewGuid(), p2, 0, "test3", -1, DateTime.UtcNow,
                                              PrepareFlags.SingleWrite, "type", new byte[0], new byte[0]),
                         out p3);
        }
