...
namespace EventStore.Core.Tests.Services.Storage.Transactions
{
    public class when_having_multievent_sequential_write_request_read_index_should : ReadIndexTestScenario
    {
        ...
        {
            var p1 = Fixture.WriteTransactionBegin("ES", ExpectedVersion.NoStream, 0, "test1");
            var p2 = Fixture.WriteTransactionEvent(p1.CorrelationId, p1.LogPosition, 1, p1.EventStreamId, 1, "test2", PrepareFlags.Data);
            var p3 = Fixture.WriteTransactionEvent(p1.CorrelationId, p1.LogPosition, 2, p1.EventStreamId, 2, "test3", PrepareFlags.TransactionEnd | PrepareFlags.Data);

            Fixture.WriteCommit(p1.CorrelationId, p1.LogPosition, p1.EventStreamId, p1.EventNumber);
