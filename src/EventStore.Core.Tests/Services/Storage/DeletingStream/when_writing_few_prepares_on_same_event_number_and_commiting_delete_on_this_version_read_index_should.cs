...
namespace EventStore.Core.Tests.Services.Storage.DeletingStream
{
    public class when_writing_few_prepares_with_same_event_number_and_commiting_delete_on_this_version_read_index_should : ReadIndexTestScenario
    {
        ...
        protected override void WriteTestScenario()
        {
            ...
            var prepare1 = LogRecord.SingleWrite(Fixture.WriterCheckpoint.ReadNonFlushed(),    // prepare1
                                                 Guid.NewGuid(),
                                                 Guid.NewGuid(),
                                                 "ES",
                                                 -1,
                                                 ...
                                                 null,
                                                 DateTime.UtcNow);
            Assert.True(Fixture.Writer.Write(prepare1, out pos));

            var prepare2 = LogRecord.SingleWrite(Fixture.WriterCheckpoint.ReadNonFlushed(),    // prepare2
                                                 Guid.NewGuid(),
                                                 Guid.NewGuid(),
                                                 "ES",
                                                 -1,
                                                 ...
                                                 null,
                                                 DateTime.UtcNow);
            Assert.True(Fixture.Writer.Write(prepare2, out pos));


            var deletePrepare = LogRecord.DeleteTombstone(Fixture.WriterCheckpoint.ReadNonFlushed(), // delete prepare
                                                          Guid.NewGuid(), Guid.NewGuid(), "ES", -1);
            var deleteTombstone = new EventRecord(EventNumber.DeletedStream, deletePrepare);
            Assert.True(Fixture.Writer.Write(deletePrepare, out pos));

            var prepare3 = LogRecord.SingleWrite(Fixture.WriterCheckpoint.ReadNonFlushed(),     // prepare3
                                                 Guid.NewGuid(),
                                                 Guid.NewGuid(),
                                                 "ES",
                                                 -1,
                                                 ...
                                                 null,
                                                 DateTime.UtcNow);
            Assert.True(Fixture.Writer.Write(prepare3, out pos));

            var commit = LogRecord.Commit(Fixture.WriterCheckpoint.ReadNonFlushed(),     // committing delete
                                          deletePrepare.CorrelationId,
                                          deletePrepare.LogPosition,
                                          EventNumber.DeletedStream);
            Assert.True(Fixture.Writer.Write(commit, out pos));
            Fixture.AddStashedValueAssignment(this, instance =>
            {
                instance._deleteTombstone = deleteTombstone;
            });
        }
