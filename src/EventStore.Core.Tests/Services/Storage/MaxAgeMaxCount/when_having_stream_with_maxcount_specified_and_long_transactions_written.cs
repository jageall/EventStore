...
namespace EventStore.Core.Tests.Services.Storage.MaxAgeMaxCount
{
    public class when_having_stream_with_maxcount_specified_and_long_transactions_written : ReadIndexTestScenario
    {
        ...
        protected override void WriteTestScenario()
        {
            ...
            WriteTransaction(Fixture, -1, 3);
            WriteTransaction(Fixture, 2, 2);
            WriteTransaction(Fixture, -1 + 3 + 2, 4);
        }

        private void WriteTransaction(FixtureData fixture, int expectedVersion, int transactionLength)
        {
            var begin = fixture.WriteTransactionBegin("ES", expectedVersion);
            for (int i = 0; i < transactionLength; ++i)
            {
                var eventNumber = expectedVersion + i + 1;
                _records[eventNumber] = fixture.WriteTransactionEvent(Guid.NewGuid(), begin.LogPosition, i, "ES", eventNumber, "data" + i, PrepareFlags.Data);
            }
            fixture.WriteTransactionEnd(Guid.NewGuid(), begin.LogPosition, "ES");
            fixture.WriteCommit(Guid.NewGuid(), begin.LogPosition, "ES", expectedVersion + 1);
        }
