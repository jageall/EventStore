...
namespace EventStore.Core.Tests.Services.Storage.MaxAgeMaxCount.ReadRangeAndNextEventNumber
{
    public class when_reading_stream_with_max_age: ReadIndexTestScenario
    {
        ...
        protected override void WriteTestScenario()
        {
            ...
            Fixture.WriteStreamMetadata("ES", 0, metadata, now.AddMinutes(-100));
            Fixture.WriteSingleEvent("ES", 0, "bla", now.AddMinutes(-50));
            Fixture.WriteSingleEvent("ES", 1, "bla", now.AddMinutes(-25));
            var event2 = Fixture.WriteSingleEvent("ES", 2, "bla", now.AddMinutes(-15));
            var event3 = Fixture.WriteSingleEvent("ES", 3, "bla", now.AddMinutes(-11));
            var event4 = Fixture.WriteSingleEvent("ES", 4, "bla", now.AddMinutes(-3));
        ...
        public when_reading_stream_with_max_age(FixtureData fixture) : base(fixture)
        {
        }
    }
}
