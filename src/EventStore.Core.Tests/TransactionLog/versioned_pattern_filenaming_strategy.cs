...
namespace EventStore.Core.Tests.TransactionLog
{
    public class versioned_pattern_filenaming_strategy: SpecificationWithDirectory
    {
        ...
        public void when_getting_file_for_positive_index_and_no_version_appends_index_to_name_with_zero_version()
        {
            ...
            Assert.Equal("path" + Path.DirectorySeparatorChar + "prefix-000001.000000", strategy.GetFilenameFor(1, 0));
        }
        ...
        {
            var strategy = new VersionedPatternFileNamingStrategy("path", "prefix-");
            Assert.Equal("path" + Path.DirectorySeparatorChar + "prefix-000001.000007", strategy.GetFilenameFor(1, 7));
        }
        ...
        public void returns_all_temp_files_in_directory()
        {
            ...
            Assert.Equal(GetFilePathFor("bla.tmp"), tempFiles[0]);
            Assert.Equal(GetFilePathFor("chunk-000005.000007.tmp"), tempFiles[1]);
            Assert.Equal(GetFilePathFor("foo.tmp"), tempFiles[2]);
        }
