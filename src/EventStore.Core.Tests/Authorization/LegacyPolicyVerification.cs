...
namespace EventStore.Core.Tests.Authorization {
	public class LegacyPolicyVerification {
		...
		public static IEnumerable<PolicyVerificationParameters> PolicyTests() {
			...
			IEnumerable<Tuple<Operation, string, StorageMessage.EffectiveAcl>> AdminOperations() {
				

				yield return new Tuple<Operation, string, StorageMessage.EffectiveAcl>(new Operation(Operations.Streams.Read).WithParameter(
						Operations.Streams.Parameters.StreamId("$$$scavenge")),
					"$$$scavenge",
					systemStreamPermission);
			}

			IEnumerable<Tuple<Operation, string, StorageMessage.EffectiveAcl>> OpsOperations() {
			...
			IEnumerable<Tuple<Operation, string, StorageMessage.EffectiveAcl>> AnonymousOperations() {
				...
				yield return CreateOperation(Operations.Node.Elections.MasterIsResigning);
				yield return CreateOperation(Operations.Node.Elections.MasterIsResigningOk);
			...
			Tuple<Operation, string, StorageMessage.EffectiveAcl> CreateOperation(OperationDefinition def) {
				return new Tuple<Operation, string, StorageMessage.EffectiveAcl>(new Operation(def),null, null );
			}
