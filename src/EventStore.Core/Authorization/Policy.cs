...
namespace EventStore.Core.Authorization
{
	public class Policy {
		...
		public void Add(OperationDefinition operation, IAssertion assertion) {
			...
			var insertAt = assertions.BinarySearch(assertion);
			if (insertAt >= 0)
		...
		public ReadOnlyPolicy AsReadOnly() {
			_sealed = true;
			return new ReadOnlyPolicy(Name, _version, ValidFrom, _assertions.ToDictionary<KeyValuePair<OperationDefinition, List<IAssertion>>, OperationDefinition, ReadOnlyMemory<IAssertion>>(x => x.Key, x => x.Value.ToArray()));
		}
		...
		public void AllowAuthenticated(in OperationDefinition operation) {
			Add(operation, new AllowAuthenticatedAssertion());
		}
	}
}