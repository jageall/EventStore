...
namespace EventStore.Core.Authorization
{
	public class PolicyAuthorizationProvider : IAuthorizationProvider {
		...
		public PolicyAuthorizationProvider(IPolicyEvaluator policyEvaluator, ILogger logger, bool logAuthorization, bool logSuccesses) {
		...
		public ValueTask<bool> CheckAccessAsync(ClaimsPrincipal cp, Operation operation, CancellationToken ct) {
			...
			try {
				...
				_logger.Error(ex, "Error performing permission check for {identity}", cp.FindFirst(ClaimTypes.Name)?.Value ?? "unknown");
				return new ValueTask<bool>(false);
			}
		}

		private async ValueTask<bool> CheckAccessAsync(TimeSpan startedAt, ClaimsPrincipal cp, ValueTask<EvaluationResult> evaluationTask) {
			try {
		...
		private bool LogAndCheck(TimeSpan startedAt, ClaimsPrincipal cp, EvaluationResult result) {
			if (_logAuthorization) {
				if (result.Grant == Grant.Allow && _logSuccesses) {
					_logger.Information("Successful authorization check for {identity} in {duration} with {evaluationResult}",
