...
namespace EventStore.Core.Authorization
{
	public sealed class AssertionComparer : IComparer<IAssertion> {
		...
		public int Compare(IAssertion x, IAssertion y) {
			...
			var closed = (Func<IAssertion, IAssertion, int>)OpenTypeComparer.MakeGenericMethod(x.GetType()).CreateDelegate(typeof(Func<IAssertion, IAssertion, int>));
			return closed(x, y);
		}

		static int Compare<T>(IAssertion x, IAssertion y) {
			
			if (x is IComparable<T> comparable)
				return comparable.CompareTo((T)y);
			throw new NotSupportedException("Assertion classes must implement IComparable<T> where T is the Assertion class");
		}
	}
}
